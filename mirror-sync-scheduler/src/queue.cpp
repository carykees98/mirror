...
void Queue::manual_sync(std::string name)
{
    ...
    std::thread t(
        [=]
        {
            ...
            {
                logger->warn(
                    name + " is not a valid project (manual sync failed)"
                );
                tLock.unlock();
                return;
            }
            ...
            if (std::find(currentJobs.begin(), currentJobs.end(), name)
                == currentJobs.end())
            {
                ...
                currentJobs.erase(
                    std::find(currentJobs.begin(), currentJobs.end(), name)
                );
            }
            else
            {
...
void Queue::jobQueueThread()
{
    while (true)
    {
        ...
        if (!queue_.empty())
        {
            ...
            if (std::find(currentJobs.begin(), currentJobs.end(), jobName)
                == currentJobs.end())
            {
                // add job to current jobs
                ...
                // erase the job from the currientJobs vector
                currentJobs.erase(
                    std::find(currentJobs.begin(), currentJobs.end(), jobName)
                );
            }
        }
        ...
        // sleep for 5 seconds so that we arnt running constantly and to prevent
        // constant locking
...
Queue::generateSyncCommands(json& config, std::string name)
{
    ...
    else if (scriptData != config.end())
    {
        ...
        std::vector<std::string> arguments
            = config["script"].value("arguments", std::vector<std::string> {});
