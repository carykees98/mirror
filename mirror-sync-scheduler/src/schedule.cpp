...
void Schedule::build(json& config)
{
    ...
    {
        // std::cout << "---------" << i << "-------------" << std::endl;
        // create a job with all tasks whos syncs per day lines up with the
        ...
        for (int j = 0; j < tasks.size(); j++)
        {
            ...
            {
                // std::cout << tasks[j].syncs << " " << tasks[j].name <<
                // std::endl;
...
bool Schedule::verify(std::vector<Task> tasks)
{
    ...
    for (int i = 0; i < tasks.size(); i++)
    {
        ...
        // std::cout << taskMap.find(tasks[i].name)->first << " " <<
        // taskMap.find(tasks[i].name)->second << " " << tasks[i].syncs <<
        // std::endl;
    }
...
std::vector<std::string>* Schedule::nextJob(int& seconds_to_sleep)
{
    ...
    // scheduleTime <= 1.0)
    double scheduleTime
        = (double)seconds_since_midnight_gmt / total_seconds_day;

    // find the first job that is greater than the currient time.
    // we start at iterator so that we dont have to search the entire job array
    // each time. we do it in this way so that if we start the program in the
    // middle of the day it will still select the correct job. in normal
    // operation it will just go through the loop once increasing the iteratior
    // to the next job.
    ...
    if (iterator == jobs.size())
    {
        ...
        // sleep till midnight
        // total seconds in a day - currient time in seconds cast to an interger
        // and added 1
        seconds_to_sleep
            = (int)(total_seconds_day - (scheduleTime * total_seconds_day)) + 1;
        return &(jobs[jobs.size() - 1].name);
    }

    // else sleep till next job
    // target time in seconds - currient time in seconds cast to an interger and
    // added 1
    seconds_to_sleep = (int)((jobs[iterator].target_time * total_seconds_day)
