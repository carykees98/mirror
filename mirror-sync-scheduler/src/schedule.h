...
// Job structs are created by the build algorithm to represent a job to be run
// at a specific time name - vector of names of all the projects that will be
// queued target_time - double between 0 and 1 representing the fraction of a
// day till its ran
struct Job
{
...
class Schedule
{
  ...
  public:
    ...
    // use the mirrors.json file to create the schedule by populating the jobs
    // vector
  ...
  private:
    ...
    // used inside build to parse the mirrors.json file into a vector of Task
    // structs
    //@param config reference to mirrors.json object
  ...
  public:
    // used inside the main function to break out of the sleep loop to
  ...
  private:
    ...
    int              iterator;
    // vector of Jobs containing names of Tasks to sync and the time to sync
    // them
    std::vector<Job> jobs;
